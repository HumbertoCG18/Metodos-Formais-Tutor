...
- Na abordagem baseada em provas, a descrição do sistema é um conjunto de fórmulas Spec (em uma determinada lógica) e a especificação é outra fórmula prop. O método de verificação consiste em tentar achar uma prova de que Spec ⊢ prop (prop é um teorema de Spec).
- Na abordagem baseada em modelos, o sistema é representado por um modelo M para uma lógica apropriada. A especificação é novamente representada por uma fórmula prop e o método de verificação consiste em computar se o modelo M satisfaz prop, isto é, se M ⊨ prop.
